...
public class Maze
{
    ...
    /**
     * Marks a given position in the maze as part of the ATTEMPT
     * ATTEMPT is used to signal that we have ATTEMPTed this space on a
     * previous solution and it can no longer be used on other solutions.
     *
     * @param row the index of the row to mark as part of the PATH
     * @param col the index of the column to mark as part of the PATH
     */
    public void solutionAttempted(int row, int col){
        grid[row][col] =  ATTEMPT;
    }
    ...
    /**
     * Determines if a specific location is valid for a solution.
     * A valid location
     * is one that is on the grid, is not blocked, and has been TRIED.
     *
     * @param row the row to be checked
     * @param column the column to be checked
     * @return true if the location is valid to be used for a solution
     */
    public boolean validSolutionPath(int row, int column){
        boolean result = false;
